...
func (r *JobReconciler) deleteStaleGangChildJobs(ctx context.Context, parent client.Object) error {
	...
	if err := r.client.List(ctx, &childJobs,
		...
		// If the index is not available (e.g., Job controller not registered),
		// log the error and silently skip — this is a best-effort cleanup.
		log.V(2).Info("Failed to list child jobs for stale gang cleanup, skipping", "error", err)
		return nil
	}
