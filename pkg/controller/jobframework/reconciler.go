...
func (r *JobReconciler) deleteStaleGangChildJobs(ctx context.Context, parent client.Object) error {
	...
	if err := r.client.List(ctx, &childJobs,
		...
		// Log but don't block eviction — the index may not exist for all job
		// types (e.g., AppWrapper). The cleanup is best-effort; blocking here
		// would prevent quota from being cleared.
		log.V(0).Info("Failed to list child jobs for stale gang cleanup, skipping", "error", err)
		return nil
	}
